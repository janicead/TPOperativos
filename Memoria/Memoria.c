...
void escuchar()
{
	...
	skEnUso = 1;
	setsockopt(socketMemoria,SOL_SOCKET,SO_REUSEADDR,&skEnUso,sizeof(skEnUso));

	if(bind(socketMemoria,(void*) &dirServidor,sizeof(dirServidor)) != 0)
	{
		perror("Fallo del bind");
		exit(0);
	}

	if(listen(socketMemoria,10)==0)
	{

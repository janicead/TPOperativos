	...
	while(1){
		...
		if(!strncmp(linea,"select",6) || !strncmp(linea,"SELECT",4)){
			...
			if(parametros[1] == NULL || string_is_empty(parametros[1])){
				log_error(loggerMemoria,"No se ha especificado un nombre de tabla");
			}
			else if(parametros[2] == NULL || string_is_empty(parametros[2])){
				log_error(loggerMemoria,"No se ha especificado ninguna key");
			}
			else{
				puts(parametros[1]);
		...
		//INSTRUCCIÓN INSERT PARÁMETROS: [NOMBRE TABLA] [KEY] "[VALUE]" [TIMESTAMP]
			...
			if(parametros[1] == NULL || string_is_empty(parametros[1])){
				...
				log_error(loggerMemoria,"No se ha especificado ningun valor");
			}
			else{
				key = atoi(parametros[2]);
		...
		//INSTRUCCIÓN CREATE PARÁMETROS: [NOMBRE TABLA] [TIPO DE CONSISTENCIA] [CANTIDAD PARTICIONES] [TIEMPO DE COMPACTACION]
			...
			if(parametros[1] == NULL || string_is_empty(parametros[1])){
				...
				log_error(loggerMemoria,"No se ha especificado ningun tipo de consistencia");
			}
			else if(parametros[3] == NULL || string_is_empty(parametros[3])){
				log_error(loggerMemoria,"No se ha especificado ninguna cantidad de particiones");
			}
			else if(parametros[4] == NULL || string_is_empty(parametros[4])){
				log_error(loggerMemoria,"No se ha especificado ningun tiempo de compactación");
			}
			else{
		...
		//INSTRUCCIÓN JOURNAL NO TIENE PARÁMETROS
			...
			log_error(loggerMemoria,"No se ingreso ningun comando valido");
		}
		free(linea);
	}
	//return NULL;
